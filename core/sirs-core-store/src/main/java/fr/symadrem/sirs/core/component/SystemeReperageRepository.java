...
@Component
@Views({
    @View(name="all",         map="function(doc) {if(doc['@class']=='fr.symadrem.sirs.core.model.SystemeReperage') {emit(doc._id, doc._id)}}"),
    @View(name="byTronconId", map="function(doc) {if(doc['@class']=='fr.symadrem.sirs.core.model.SystemeReperage') {emit(doc.tronconId, doc._id)}}")
})
