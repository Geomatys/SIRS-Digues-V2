 ...
 */
class OuvrageRevancheImporter extends GenericStructureImporter<OuvrageRevanche> {

    private Map<Integer, OuvrageRevanche> ouvrages = null;
    private Map<Integer, List<OuvrageRevanche>> ouvragesByTronconId = null;

    OuvrageRevancheImporter(final Database accessDatabase,
    ...
    protected void compute() throws IOException, AccessDbImporterException {
        ...
        while (it.hasNext()) {
            ...
            if(row.getInt(FondationColumns.ID_TYPE_COTE.toString())!=null){
                ouvrage.setCoteId(typesCote.get(row.getInt(FondationColumns.ID_TYPE_COTE.toString())).getId());
            }
            
            if(row.getInt(FondationColumns.ID_SOURCE.toString())!=null){
                ouvrage.setSourceId(typesSource.get(row.getInt(FondationColumns.ID_SOURCE.toString())).getId());
            }
            ...
            if (row.getDate(FondationColumns.DATE_DEBUT_VAL.toString()) != null) {
                ouvrage.setDate_debut(LocalDateTime.parse(row.getDate(FondationColumns.DATE_DEBUT_VAL.toString()).toString(), dateTimeFormatter));
            }
            
            if (row.getDate(FondationColumns.DATE_FIN_VAL.toString()) != null) {
                ouvrage.setDate_fin(LocalDateTime.parse(row.getDate(FondationColumns.DATE_FIN_VAL.toString()).toString(), dateTimeFormatter));
            }
            
            if (row.getDouble(FondationColumns.PR_DEBUT_CALCULE.toString()) != null) {
                ouvrage.setPR_debut(row.getDouble(FondationColumns.PR_DEBUT_CALCULE.toString()).floatValue());
            }
            
            if (row.getDouble(FondationColumns.PR_FIN_CALCULE.toString()) != null) {
                ouvrage.setPR_fin(row.getDouble(FondationColumns.PR_FIN_CALCULE.toString()).floatValue());
            }
            ...
            try {
                ...
                try {
                    ...
                    Logger.getLogger(OuvrageRevancheImporter.class.getName()).log(Level.SEVERE, null, ex);
                }
            ...
            if (row.getInt(FondationColumns.ID_SYSTEME_REP.toString()) != null) {
                ouvrage.setSystemeRepId(systemesReperage.get(row.getInt(FondationColumns.ID_SYSTEME_REP.toString())).getId());
            }
            
            if (row.getDouble(FondationColumns.ID_BORNEREF_DEBUT.toString()) != null) {
                ouvrage.setBorneDebutId(bornes.get((int) row.getDouble(FondationColumns.ID_BORNEREF_DEBUT.toString()).doubleValue()).getId());
            }
            
            ouvrage.setBorne_debut_aval(row.getBoolean(FondationColumns.AMONT_AVAL_DEBUT.toString()));
            
            if (row.getDouble(FondationColumns.DIST_BORNEREF_DEBUT.toString()) != null) {
                ouvrage.setBorne_debut_distance(row.getDouble(FondationColumns.DIST_BORNEREF_DEBUT.toString()).floatValue());
            }
            
            if (row.getDouble(FondationColumns.ID_BORNEREF_FIN.toString()) != null) {
                ouvrage.setBorneFinId(bornes.get((int) row.getDouble(FondationColumns.ID_BORNEREF_FIN.toString()).doubleValue()).getId());
            }
            
            ouvrage.setBorne_fin_aval(row.getBoolean(FondationColumns.AMONT_AVAL_FIN.toString()));
            
            if (row.getDouble(FondationColumns.DIST_BORNEREF_FIN.toString()) != null) {
                ouvrage.setBorne_fin_distance(row.getDouble(FondationColumns.DIST_BORNEREF_FIN.toString()).floatValue());
            }
            
            ouvrage.setCommentaire(row.getString(FondationColumns.COMMENTAIRE.toString()));
            
            if(row.getInt(FondationColumns.ID_TYPE_NATURE_HAUT.toString())!=null){
                ouvrage.setNatureHautId(typesNature.get(row.getInt(FondationColumns.ID_TYPE_NATURE_HAUT.toString())).getId());
            }
            
            if(row.getInt(FondationColumns.ID_TYPE_MATERIAU_HAUT.toString())!=null){
                ouvrage.setMateriauHautId(typesMateriau.get(row.getInt(FondationColumns.ID_TYPE_MATERIAU_HAUT.toString())).getId());
            }
            
            if(row.getInt(FondationColumns.ID_TYPE_MATERIAU_BAS.toString())!=null){
                ouvrage.setMateriauBasId(typesMateriau.get(row.getInt(FondationColumns.ID_TYPE_MATERIAU_BAS.toString())).getId());
            }
            
            if(row.getInt(FondationColumns.ID_TYPE_NATURE_BAS.toString())!=null){
                ouvrage.setNatureBasId(typesNature.get(row.getInt(FondationColumns.ID_TYPE_NATURE_BAS.toString())).getId());
            }
            
            if(row.getInt(FondationColumns.ID_TYPE_POSITION.toString())!=null){
                ouvrage.setPosition_structure(typesPosition.get(row.getInt(FondationColumns.ID_TYPE_POSITION.toString())).getId());
            }
            

            // Don't set the old ID, but save it into the dedicated map in order to keep the reference.
            //tronconDigue.setId(String.valueOf(row.getString(TronconDigueColumns.ID.toString())));
            ouvrages.put(row.getInt(FondationColumns.ID_ELEMENT_STRUCTURE.toString()), ouvrage);

            // Set the list ByTronconId
            List<OuvrageRevanche> listByTronconId = ouvragesByTronconId.get(row.getInt(FondationColumns.ID_TRONCON_GESTION.toString()));
            if (listByTronconId == null) {
                listByTronconId = new ArrayList<>();
                ouvragesByTronconId.put(row.getInt(FondationColumns.ID_TRONCON_GESTION.toString()), listByTronconId);
            }
