...
//        ID_AUTO
    ...
    public Epi importRow(Row row) throws IOException, AccessDbImporterException {
        ...
        if (row.getDate(Columns.DATE_DEBUT_VAL.toString()) != null) {
            epi.setDate_debut(DbImporter.parseLocalDate(row.getDate(Columns.DATE_DEBUT_VAL.toString()), dateTimeFormatter));
        }

        if (row.getDate(Columns.DATE_FIN_VAL.toString()) != null) {
            epi.setDate_fin(DbImporter.parseLocalDate(row.getDate(Columns.DATE_FIN_VAL.toString()), dateTimeFormatter));
        }

        if (row.getDouble(Columns.PR_DEBUT_CALCULE.toString()) != null) {
            epi.setPrDebut(row.getDouble(Columns.PR_DEBUT_CALCULE.toString()).floatValue());
        }

        if (row.getDouble(Columns.PR_FIN_CALCULE.toString()) != null) {
            epi.setPrFin(row.getDouble(Columns.PR_FIN_CALCULE.toString()).floatValue());
        }
        ...
        epi.setGeometry(buildGeometry(troncon.getGeometry(), epi, tronconGestionDigueImporter.getBorneDigueRepository()));
