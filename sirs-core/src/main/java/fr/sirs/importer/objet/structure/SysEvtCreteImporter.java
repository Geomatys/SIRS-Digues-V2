...
//     EPAISSEUR_Y22,
    ...
    protected void compute() throws IOException, AccessDbImporterException {
        ...
        while (it.hasNext()) {
            ...
            final Crete crete = importRow(row);
    ...
    public Crete importRow(Row row) throws IOException, AccessDbImporterException {
        ...
        if (row.getDate(Columns.DATE_DEBUT_VAL.toString()) != null) {
            crete.setDate_debut(DbImporter.parse(row.getDate(Columns.DATE_DEBUT_VAL.toString()), dateTimeFormatter));
        }

        if (row.getDate(Columns.DATE_FIN_VAL.toString()) != null) {
            crete.setDate_fin(DbImporter.parse(row.getDate(Columns.DATE_FIN_VAL.toString()), dateTimeFormatter));
        }
