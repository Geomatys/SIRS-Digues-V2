 ...
 */
class SysEvtReseauEauImporter extends GenericObjetImporter<ReseauHydroCielOuvert> {
    
    private final TypeReseauEauImporter typeReseauEauImporter;

    SysEvtReseauEauImporter(final Database accessDatabase,
...
//        ID_AUTO
    ...
    public String getTableName() {
        return DbImporter.TableName.SYS_EVT_RESEAU_EAU.toString();
    }
    ...
    protected void compute() throws IOException, AccessDbImporterException {
        ...
        final Map<Integer, RefReseauHydroCielOuvert> typesReseauHydroCielOuvert = typeReseauEauImporter.getTypes();
        ...
        while (it.hasNext()) {
            ...
            if(row.getInt(Columns.ID_TYPE_COTE.toString())!=null){
                reseauEau.setCoteId(typesCote.get(row.getInt(Columns.ID_TYPE_COTE.toString())).getId());
            }
            ...
            if (troncon.getId() != null) {
                reseauEau.setTroncon(troncon.getId());
            } else {
            ...
            if (row.getDate(Columns.DATE_DEBUT_VAL.toString()) != null) {
                reseauEau.setDate_debut(LocalDateTime.parse(row.getDate(Columns.DATE_DEBUT_VAL.toString()).toString(), dateTimeFormatter));
            }
            
            if (row.getDate(Columns.DATE_FIN_VAL.toString()) != null) {
                reseauEau.setDate_fin(LocalDateTime.parse(row.getDate(Columns.DATE_FIN_VAL.toString()).toString(), dateTimeFormatter));
            }
            ...
            if (row.getInt(Columns.ID_SYSTEME_REP.toString()) != null) {
                reseauEau.setSystemeRepId(systemesReperage.get(row.getInt(Columns.ID_SYSTEME_REP.toString())).getId());
            }
            
            if (row.getDouble(Columns.ID_BORNEREF_DEBUT.toString()) != null) {
                reseauEau.setBorneDebutId(bornes.get((int) row.getDouble(Columns.ID_BORNEREF_DEBUT.toString()).doubleValue()).getId());
            }
            
            reseauEau.setBorne_debut_aval(row.getBoolean(Columns.AMONT_AVAL_DEBUT.toString()));
            
            if (row.getDouble(Columns.DIST_BORNEREF_DEBUT.toString()) != null) {
                reseauEau.setBorne_debut_distance(row.getDouble(Columns.DIST_BORNEREF_DEBUT.toString()).floatValue());
            }
            
            if (row.getDouble(Columns.ID_BORNEREF_FIN.toString()) != null) {
                if(bornes.get((int) row.getDouble(Columns.ID_BORNEREF_FIN.toString()).doubleValue())!=null){
                    reseauEau.setBorneFinId(bornes.get((int) row.getDouble(Columns.ID_BORNEREF_FIN.toString()).doubleValue()).getId());
                }
            }
            
            reseauEau.setBorne_fin_aval(row.getBoolean(Columns.AMONT_AVAL_FIN.toString()));
            
            if (row.getDouble(Columns.DIST_BORNEREF_FIN.toString()) != null) {
                reseauEau.setBorne_fin_distance(row.getDouble(Columns.DIST_BORNEREF_FIN.toString()).floatValue());
            }
