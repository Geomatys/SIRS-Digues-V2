...
public class FXMapContextTree extends BorderPane {
    ...
    /**
     * CSS Style class applied when a node is dragged on the top border of another one.
     */
    public static final String ABOVE_CSS = "dragged-above";

    /**
     * CSS Style class applied when a node is dragged on the bottom border of another one.
     */
    ...
    /**
     * Analyze if cursor is positionned on the upper or lower bound of target node.
     *
     * @param pick Result of proceed picking.
     * @return {@link #DRAGGED_ABOVE} if picking is located on upper target bound,
     * {@link #DRAGGED_BELOW} if it's on lower bound. Otherwise, {@link #DRAGGED_INTO}
     * is sent back.
     */
