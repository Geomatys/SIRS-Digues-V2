...
public class DbImporter {
    ...
    */
    ...
    private DbImporter() throws IOException {
        ...
        this.dateTimeFormatter = DateTimeFormatter.ofPattern("EEE MMM dd HH:mm:ss z yyyy", Locale.FRENCH);
    }
    ...
    public Database getDatabase(){return this.accessDatabase;}
    ...
    */
    private static enum DigueColumns {
        ...
        private DigueColumns(final String column){this.column = column;}
        @Override
        public String toString(){return this.column;}
    };
    ...
    private List<Digue> getDigues() throws IOException {
        ...
        final Table tableDigue = this.accessDatabase.getTable("DIGUE");
        
        final Iterator<Row> it = tableDigue.iterator();
        while(it.hasNext()){
            ...
            if(row.getDate(DigueColumns.MAJ.toString()) != null){
                digue.setDateMaj(LocalDateTime.parse(row.getDate(DigueColumns.MAJ.toString()).toString()));
            }
    ...
    * DATE_DERNIERE_MAJ
    */
    private static enum TronconDigueColumns {
        ...
        protected final String column;
        private TronconDigueColumns(final String column){this.column = column;}
        @Override
        public String toString(){return this.column;}
    };
    ...
    private List<TronconDigue> getTronconsDigues() throws IOException {
        ...
        final Table tableTroncons = this.accessDatabase.getTable("TRONCON_GESTION_DIGUE");
        
        //tableTroncons.getColumns().stream().forEach((column) -> {System.out.println(column.getName());});
        
        final Iterator<Row> it = tableTroncons.iterator();
        while(it.hasNext()){
            final Row row = it.next();
            System.out.println(row);
            final TronconDigue tronconDigue = new TronconDigue();
            tronconDigue.setNom(row.getString(TronconDigueColumns.NOM.toString()));// Le libellé devrait devenir le nom dans une prochaine version du modèle
            tronconDigue.setCommentaire(row.getString(TronconDigueColumns.COMMENTAIRE.toString()));
            if(row.getDate(TronconDigueColumns.MAJ.toString()) != null){
                tronconDigue.setDateMaj(LocalDateTime.parse(row.getDate(TronconDigueColumns.MAJ.toString()).toString()));
            }
//            if(row.getDate(TronconDigueColumns.DEBUT_VAL_TRONCON.toString()) != null){
//                tronconDigue.setDate_debut(LocalDateTime.parse(row.getDate(TronconDigueColumns.DEBUT_VAL_TRONCON.toString()).toString()));
//            }
//            if(row.getDate(TronconDigueColumns.FIN_VAL_TRONCON.toString()) != null){
//                tronconDigue.setDate_fin(LocalDateTime.parse(row.getDate(TronconDigueColumns.FIN_VAL_TRONCON.toString()).toString(), dateTimeFormatter));
//            }
//            if(row.getString(TronconDigueColumns.DEBUT_VAL_GESTIONNAIRE.toString()) != null){
//                tronconDigue.setDate_maj(LocalDateTime.parse(row.getString(TronconDigueColumns.DEBUT_VAL_GESTIONNAIRE.toString())));
//            }
//            if(row.getString(TronconDigueColumns.FIN_VAL_GESTIONNAIRE.toString()) != null){
//                tronconDigue.setDate_maj(LocalDateTime.parse(row.getString(TronconDigueColumns.FIN_VAL_GESTIONNAIRE.toString())));
//            }
            
            // Don't set the old ID, but save it into the dedicated map in order to keep the reference.
            //tronconDigue.setId(String.valueOf(row.getString(TronconDigueColumns.ID.toString())));
            tronconDigueIds.put(row.getInt(TronconDigueColumns.ID.toString()), tronconDigue);
            
            // Set the references.
            tronconDigue.setDigueId(digueIds.get(row.getInt(TronconDigueColumns.DIGUE.toString())).getId());
    ...
    public void importTronconsDigues() throws IOException {
        this.getTronconsDigues().stream().forEach((tronconDigue) -> {
            System.out.println(tronconDigue);
