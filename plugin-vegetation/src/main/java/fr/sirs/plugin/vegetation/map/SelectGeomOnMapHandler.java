...
import static fr.sirs.plugin.vegetation.map.CreateVegetationPolygonTool.getParcelleConstraints;
 ...
 */
class SelectGeomOnMapHandler extends AbstractNavigationHandler {

    private static final double POINT_RADIUS = 1.;
    private AbstractMouseHandler mouseListener;
    ...
    public SelectGeomOnMapHandler(final ZoneVegetation vegetation, final ParcelleVegetation parcelle) {
        super();
        ArgumentChecks.ensureNonNull("Zone de végétation", vegetation);
        ArgumentChecks.ensureNonNull("Parcelle", parcelle);
        mouseListener = new GeometryMouseListener(anchor, selection, vegetation, parcelle);
    }
...
//            final Point anchor = new Point();
    ...
    private class GeometryMouseListener extends FXPanMouseListen {

        final ContextMenu choice = new ContextMenu();
        private final Geometry constraint;
        //Attributs liés au carré de sélection.
        Rectangle selection;
        Light.Point anchor;

        Geometry selectedGeometry;
        ZoneVegetation vegetation;
        String parcelleId;

        public GeometryMouseListener(final Light.Point anchor, final Rectangle selection, final ZoneVegetation vegetation, final ParcelleVegetation parcelle) {
            super(SelectGeomOnMapHandler.this);
            ArgumentChecks.ensureNonNull("Zone de végétation", vegetation);
            ArgumentChecks.ensureNonNull("Parcelle Id", parcelle);
            choice.setAutoHide(true);
            this.selection = selection;
            this.anchor = anchor;
            this.vegetation = vegetation;
            this.parcelleId = parcelle.getId();
            this.constraint = getParcelleConstraints(parcelle); //TODO
        }
